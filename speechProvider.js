...
async function startNativeSpeechRecognition(options = {}) {
    ...
    const ready = await prepareNativeSpeechRecognition(plugin, options);
    if (!ready) return null;

    const listeners = [];

    if (typeof plugin.addListener === "function") {
        await addNativeResultListeners(plugin, listeners, options);
    }
...
async function prepareNativeSpeechRecognition(plugin, options) {
    try {
        const availability = await callNativeOptional(plugin, "available");
        if (availability === false || availability?.available === false) {
            options.onError?.({ error: "native-unavailable" });
            return false;
        }

        const permission = await callNativeOptional(plugin, "checkPermissions");
        if (isPermissionDenied(permission)) {
            if (typeof plugin.requestPermissions !== "function") {
                options.onError?.({ error: "not-allowed" });
                return false;
            }
            const requested = await callNativeOptional(plugin, "requestPermissions");
            if (isPermissionDenied(requested)) {
                options.onError?.({ error: "not-allowed" });
                return false;
            }
        }

        await callNativeOptional(plugin, "isOnDeviceRecognitionAvailable");
        return true;
    } catch (error) {
        options.onError?.(error);
        return false;
...
async function addNativeResultListeners(plugin, listeners, options) {
    const partialEventNames = ["partialResults", "partialResult", "partialResultsChanged", "speechPartialResult"];
    const finalEventNames = ["segmentResults", "result", "results", "speechResult", "speechRecognitionResult"];
    const errorEventNames = ["error", "speechError", "speechRecognitionError"];
    const endEventNames = ["listeningState", "end", "stop", "speechEnd", "speechRecognitionEnd"];

    for (const eventName of partialEventNames) {
        listeners.push(await addNativeListener(plugin, eventName, (event) => {
    ...
    for (const eventName of errorEventNames) {
        listeners.push(await addNativeListener(plugin, eventName, (event) => {
    ...
    for (const eventName of endEventNames) {
        listeners.push(await addNativeListener(plugin, eventName, (event) => {
            if (eventName === "listeningState" && !isNativeStoppedState(event)) return;
            options.onEnd?.(event);
        }));
    }
}

async function callNativeOptional(plugin, methodName, payload) {
    if (typeof plugin?.[methodName] !== "function") return null;
...
function isPermissionDenied(permission) {
    ...
    return values.some((value) => value === "denied" || value === "prompt" || value === "prompt-with-rationale");
}

function isNativeStoppedState(event) {
    const state = event?.status || event?.state || event?.listeningState;
    return state === "stopped" || state === "stoppingListening" || state === "ended";
}
